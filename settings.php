...
if ($ADMIN->fulltree) {
    ...
    foreach($courserolearray as $roleid => $rolename) {
        $rolelist[] = html_writer::link(
            new \moodle_url('/admin/roles/define.php', ['action' => 'edit', 'roleid' => $roleid]), $rolename);
    }
    ...
    $settings->add(new admin_setting_description('publisher_role_mapping', get_string('publisherrolemapping', 'block_panopto'), $description));
    ...
    $description = implode(', ', $rolelist) . '<br>' . get_string('provisionrole_help', 'block_panopto', $a);
    $settings->add(new admin_setting_description('creator_role_mapping', get_string('creatorrolemapping', 'block_panopto'), $description));
